...
export default function ScoreOverlayBaseball({ game, recentEvent }) {
  ...
  return (
    <div className="absolute top-3 left-3 z-20 pointer-events-none">
      <div className="bg-black/85 backdrop-blur-sm rounded-xl shadow-lg overflow-hidden min-w-[200px]">
        {/* Teams & Score */}
        ...
        {/* Inning + Count */}
        <div className="bg-white/10 px-3 py-1.5 flex items-center justify-between gap-3">
          {/* Inning */}
          <div className="text-white text-xs font-bold">
            <span className="text-yellow-400">{halfLabel}</span> {inningNum}
          </div>
          ...
          <div className="flex items-center gap-2 text-[10px] text-slate-300">
            <div className="flex items-center gap-0.5">
              <span>B</span>
              {[0, 1, 2, 3].map(i => (
                <div key={i} className={`w-2 h-2 rounded-full ${i < balls ? "bg-green-400" : "bg-slate-600"}`} />
              ))}
            </div>
            <div className="flex items-center gap-0.5">
              <span>S</span>
              {[0, 1, 2].map(i => (
                <div key={i} className={`w-2 h-2 rounded-full ${i < strikes ? "bg-yellow-400" : "bg-slate-600"}`} />
              ))}
            </div>
            <div className="flex items-center gap-0.5">
              <span>O</span>
              {[0, 1, 2].map(i => (
                <div key={i} className={`w-2 h-2 rounded-full ${i < outs ? "bg-red-400" : "bg-slate-600"}`} />
              ))}
            </div>
          </div>
          ...
          <div className="relative w-6 h-6 flex-shrink-0">
            {/* 2nd base (top) */}
            <div className={`absolute top-0 left-1/2 -translate-x-1/2 w-2 h-2 rotate-45 ${bases[1] ? "bg-yellow-400" : "bg-slate-600"}`} />
            {/* 3rd base (left) */}
            <div className={`absolute top-1/2 left-0 -translate-y-1/2 w-2 h-2 rotate-45 ${bases[2] ? "bg-yellow-400" : "bg-slate-600"}`} />
            {/* 1st base (right) */}
            <div className={`absolute top-1/2 right-0 -translate-y-1/2 w-2 h-2 rotate-45 ${bases[0] ? "bg-yellow-400" : "bg-slate-600"}`} />
          </div>
        </div>
      </div>
